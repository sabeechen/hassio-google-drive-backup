...
for %%p in (armhf armv7 aarch64 amd64 i386) do (
	docker build --pull -t "sabeechen/hassio-google-drive-backup-%%p:%v%" --label "io.hass.version=%v%" --build-arg "BUILD_FROM=homeassistant/armv7-base:latest" --build-arg "BUILD_VERSION=%v%" --label "io.hass.arch=%%p" --build-arg "BUILD_ARCH=%%p" hassio-google-drive-backup/
